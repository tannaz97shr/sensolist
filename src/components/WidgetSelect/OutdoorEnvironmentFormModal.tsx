...
export default function ChartFormModal({
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<IOutdoorEnvironmentFormData>({
    values: values
      ? values
      : { title: "", thing: thingsList[0] ? thingsList[0].value : "" },
  });

  const onSubmit: SubmitHandler<IOutdoorEnvironmentFormData> = (data) => {
